  ...
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', async () => {
      try {
        ...
        console.error('ServiceWorker registration fail ', error);
      }
    });
  }
...
async function getRecipes() {
  ...
  const recipesFound = localStorage.getItem('recipes');
  if (recipesFound) {
    return JSON.parse(recipesFound);
  }
  /**************************/
  ...
  try {
    ...
    console.error('Error while trying to fetch recipes ', error);
    reject(error);
  }});
  /**************************/
